...
/// Service identifier under which all chajá tokens live. Keep this
/// stable — changing it would orphan every existing token in the
/// keyring.
const KEYRING_SERVICE: &str = "io.chaja.integrations";
...
fn map_err(e: keyring::Error) -> BackendError {
    ...
    match e {
        KE::PlatformFailure(inner) | KE::NoStorageAccess(inner) => {
            ...
            // keyring service itself is the problem, not chajá.
