...
use crate::backend::{
    BackendError, BranchInfo, BranchKind, CommitDiff, DiffHunk, DiffLine, FileDiff, FileStatus,
    GitBackend, LineKind, MergeResult, MergeStrategy, RepoStateInfo, DIFF_MAX_FILE_BYTES,
};
    ...
    fn commit_diff(&self, repo_path: &Path, sha: &str) -> Result<CommitDiff, BackendError> {
        ...
        for idx in 0..delta_count {
            let delta = diff.get_delta(idx).ok_or_else(|| {
                BackendError::Git(anyhow::anyhow!("delta at index {idx} missing"))
            })?;
...
/// Shared credential callback used by push / fetch. Tries ssh-agent first,
/// then the user's configured git credential helper (gh cli, osxkeychain,
/// libsecret, wincred, pass, …), then the default. Returns a typed error
/// when none apply.
fn build_credentials_callbacks() -> git2::RemoteCallbacks<'static> {
