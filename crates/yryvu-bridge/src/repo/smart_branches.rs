...
//!    - PR base branch — yryvu has no forge integration yet, so this is
//!      always [`None`] today.
...
/// open. yryvu deviates from GK here: GK silently no-ops, while we surface
...
mod tests {
    ...
    fn init_repo(name: &str) -> tempfile::TempDir {
        ...
        run(dir.path(), &["config", "user.email", "test@yryvu.local"]);
    ...
    fn single_branch_repo_returns_only_head() {
        let repo = init_repo("yryvu-smart-single");
    ...
    fn detached_head_returns_empty() {
        let repo = init_repo("yryvu-smart-detached");
    ...
    fn head_plus_default_branch_main() {
        let repo = init_repo("yryvu-smart-default-main");
    ...
    fn reflog_base_wins_over_default_branch() {
        let repo = init_repo("yryvu-smart-reflog");
    ...
    fn profile_default_branch_used_when_remote_head_absent() {
        let repo = init_repo("yryvu-smart-profile-default");
    ...
    fn default_branch_resolves_to_main_when_present() {
        let repo = init_repo("yryvu-smart-default-resolve");
    ...
    fn default_branch_resolves_to_master_when_main_missing() {
        let dir = tempfile::TempDir::with_prefix("yryvu-smart-master").expect("tempdir");
    ...
    fn default_branch_degenerate_fallback_when_nothing_resolves() {
        let dir = tempfile::TempDir::with_prefix("yryvu-smart-empty").expect("tempdir");
    ...
    fn tags_never_appear_in_visible_set() {
        let repo = init_repo("yryvu-smart-tags");
