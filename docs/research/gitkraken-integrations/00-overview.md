...
This audit covers everything yryvu needs to clone the surface 1:1
...
| 09 | `09-out-of-scope-proprietary.md` | what yryvu MUST skip |
| 10 | `10-yryvu-implementation-hints.md` | translating to Tauri+SolidJS |
...
### Auth modes — yryvu v1 decision

GK supports 3 enums (`OAUTH` / `PAT` / `USERNAME_AND_PASSWORD`).
yryvu ships **2**:

- **Primary**: OAuth via PKCE, direct to each provider (yryvu registers
  its own OAuth apps; no GK auth proxy — see `03-oauth-flow.md`).
- **Fallback**: PAT — universal across every provider yryvu supports.
...
vs yryvu v1" for the full per-provider matrix.
...
- The **GK auth proxy** (`buildAuthUrl` at `bundle:52634` routes
  OAuth through `${apiUrl}/oauth/${authEndpointName}/login`). yryvu
...
- `requiresProToAuthenticate: true` (`bundle:166447`,
  `166576`, `166702`) — paywall on self-hosted variants. yryvu has
...
## yryvu deviation: no GK auth proxy

GK funnels OAuth through `${getApiUrl}/oauth/<provider>/login` — a
service it controls. yryvu must talk OAuth directly to each provider's
...
what yryvu has to reimplement raw.
