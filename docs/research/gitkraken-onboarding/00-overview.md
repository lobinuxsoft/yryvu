...
files). Targets chajá issue #100 (`feat(onboarding): open / clone / init
repo dialogs`).
...
chajá already mirrors that shape: `ColdStart/index.tsx:33-37` and
...
1. **First-run welcome screen / FTUX** (post-login). Big buttons, big
   ...
   Equivalent in chajá: `ColdStart/index.tsx`.
2. **Repo Management permanent tab**. Top-of-list action row:
   Open / Clone / Init. Equivalent in chajá:
...
chajá needs to skip and `03-clone-form.md` / `06-init-form.md` for the
tab structures.
...
| 01-choose-form.md | OnboardingChooseRepoForm (folder scanner — chajá: defer) |
| 02-open-form.md | OnboardingOpenRepoForm (chajá has it, gap analysis) |
...
| 12-chaja-implementation-hints.md | Wiring recipes for #100 |
...
|---|---|---|
| Open existing local repo | **DONE** | chajá `ColdStart` + `RepoManagement` already wire this via `@tauri-apps/plugin-dialog`. No changes needed for #100 unless we want gap-fix items from doc 02. |
...
| LFS init toggle | **FLAG / defer** | GK shows a checkbox if `isLfsInstalled` (`bundle:286105`). chajá has no LFS yet. Defer. |
...
| Recurse-submodules toggle | **FLAG / off-by-default** | GK reads from `getAutoUpdateSubmodules` profile setting (`bundle:131706`). chajá: a single checkbox in Clone form, default ON, mirroring GK's default at `bundle:188608` (`recurseSubmodules: ua.recurseSubmodules ?? !0`). |
| "Open in new tab" after clone/init | **KEEP** | GK default is `OPEN_IN_NEW_TAB` (`bundle:188504`). chajá: call `openRepoInAnotherTab` from `tabs/ops.ts` post-success — same path `ColdStart` uses today. |
...
1. **Hosting-service tabs are SKIP v1.** GK's URL-tab is part of a tab
   container; chajá's clone form should render only the URL pane until
   D3+ provider clients land.
2. **`OnboardingChooseRepoForm` folder-scanner has no chajá analogue.**
...
3. **Init form is shaped around hosting-service create-and-push.** The
   Local tab is only one of N. chajá renders only Local, no tab chrome
   needed in v1.
