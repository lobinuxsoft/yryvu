...
Yryvu uses Tauri, so the OS-accelerator side is replaced by global `keydown` listeners on `window` (the same pattern already used for undo/redo at `apps/yryvu-app/src/components/AppShell.tsx`, see `feedback`/undo cluster #130).
